...
class LitGeoTrTemplateLarge(pl.LightningModule):
    ...
    train_options = {
        "max_epochs": 300,
        "batch_size": 4,  # virtual batch size of 8 (with two GPUs)
    ...
    def __init__(self):
        super().__init__()
        self.model = GeoTrTemplateLarge(num_attn_layers=6)
        self.epochs = None
...
class EfficientNetEncoder(nn.Module):
    ...
    def forward(self, image):
        ...
        x = self.backbone(image)  # N, 160, 38, 38
        x = self.relu(x)
        x = self.reduce_conv(x)  # N, #out, 36, 36
        return x


class GeoTrTemplateLarge(nn.Module):
    def __init__(self, num_attn_layers):
        super(GeoTrTemplateLarge, self).__init__()
        self.num_attn_layers = num_attn_layers

        self.hidden_dim = hdim = 256

        self.fnet1 = EfficientNetEncoder(output_dim=128)
        self.fnet2 = EfficientNetEncoder(output_dim=128)
